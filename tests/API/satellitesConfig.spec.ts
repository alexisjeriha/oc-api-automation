...
const successfullConfigCreationMessage = "Mission config created successfully";
...
interface ConfigResponse {
  meta: null;
  data: SatelliteConfig[] | { message: string } | null;
  errors: APIError[] | null;
}

async function getAllExistingSatelliteConfigurations(request) {
  const response = await request.get(`http://${localEnvURL}/${satelliteConfigurationsEndpoint}`);
...
async function deleteAllExistingSatelliteConfigurations(request) {
  ...
  ids.forEach(async (id) => {
    await request.delete(`http://${localEnvURL}/${satelliteConfigurationsEndpoint}/${id}`);
  });
...
test.describe("POST /configs", () => {
  test("Create Single Valid Satellite Configuration", async ({ request }) => {
    const payloadSatelliteConfig = { name: "Test Satellites 322", type: "OPTICAL", cospar_id: "2023-001AB" };

    const expectedSatelliteConfigsData = [{ id: 1, name: "Test Satellites 322", type: "OPTICAL", cospar_id: "2023-001AB" }];

    const { response, data } = await createSingleSatelliteConfiguration(request, payloadSatelliteConfig);
    ...
    test("Create Multiple Valid Satellite Configuration", async ({ request, }) => {
      const payloadSatelliteConfigs = [
        { name: "Test Satellites 322", type: "OPTICAL", cospar_id: "2023-001AB" },
      ...
      const expectedSatelliteConfigsData = [
        { id: 1, name: "Test Satellites 322", type: "OPTICAL", cospar_id: "2023-001AB" },
        { id: 2, name: "Test Satellites 3643", type: "SAR", cospar_id: "2056-231ZX" }
      ];
  ...
  test.skip("Create Satellite Configuration with Invalid Field Name", async ({ request }) => { }),

    test("Create Satellite Configuration without Required Fields", async ({ request }) => {
      const payloadSatelliteConfigWithoutName = { type: "OPTICAL", cospar_id: "2023-001AB" }
  ...
  test.describe("DELETE /configs/:missionID", () => {
    ...
    test.beforeEach(async ({ request }) => {
      const payloadSatelliteConfig = { name: "Dummy Valid Satellite", type: "OPTICAL", cospar_id: "2023-001AB" }
      await createSingleSatelliteConfiguration(request, payloadSatelliteConfig);
    })

    test("Delete Existing Satellite Configuration", async ({ request }) => {
      const existingSatelliteConfigId = 1
      const successfullConfigDeletionMessage = "Mission config deleted successfully"
      const response = await request.delete(`http://${localEnvURL}/${satelliteConfigurationsEndpoint}/${existingSatelliteConfigId}`);
      const body: ConfigResponse = await response.json();
      const data = body.data;
      ...
      expect(body.meta).toBeNull()
      expect(body.errors).toBeNull()
      expect(data?.message).toEqual(successfullConfigDeletionMessage);
    });
...
test.describe("Other Edge Cases", () => {
  test("Invalid Endpoint", async ({ request }) => {
    ...
    const response = await request.get(`http://${localEnvURL}/${invalidEndpoint}`);
    const body: ConfigResponse = await response.json();
    const data = body.data;
  ...
  test("Service Limit Exceeded", async ({ request }) => {
    ...
    const serviceLimitExceededMessage = "invalid request due to mission config database is full"
    ...
    const { response: responseExceeding, body: bodyExceeding } = await createSingleSatelliteConfiguration(request, payloadSatelliteConfigExtra);

    expect(responseExceeding.status()).toBe(400);
    expect(bodyExceeding.errors?.[0].message).toEqual(serviceLimitExceededMessage);
  });
